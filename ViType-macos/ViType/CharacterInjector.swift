...
enum InjectionMethod {
    ...
    case extraSlow      // TUI apps: higher delays for slow event-loop apps
    ...
    var delays: (backspace: UInt32, wait: UInt32, text: UInt32) {
        ...
        case .slow:         return (8000, 15000, 8000)      // 8ms/15ms/8ms for Terminals
        case .extraSlow:    return (12000, 4000, 12000)     // 12ms/4ms/12ms for TUI apps
...
class CharacterInjector {
    ...
    /// Common shell process names — any non-shell descendant of a terminal is
    /// considered a TUI app that needs slower injection. This replaces the old
    /// hardcoded app-name approach with universal detection.
    private let knownShells: Set<String> = [
        "bash", "zsh", "fish", "sh", "dash", "tcsh", "csh", "ksh",
        "nu", "pwsh", "elvish", "xonsh", "ion", "oil", "osh",
        "login", "sshd", "tmux", "screen", "zellij", "mosh-server",
    ]

    /// All terminal emulator bundle IDs
    ...
    /// Check if the frontmost terminal is running a TUI app (any non-shell process).
    /// Scans leaf processes in the terminal's process tree — if any leaf is not a
    /// known shell/multiplexer, it is treated as a TUI app.
    /// Result is cached for 2 seconds to minimize overhead.
    private func isTerminalRunningTUI() -> Bool {
        ...
        cachedTUIResult = hasNonShellDescendant(parentPid: terminalPid, maxDepth: 5)
        return cachedTUIResult
    }

    /// BFS scan of process tree. Returns true if any leaf (or near-leaf) process
    /// is NOT a known shell/multiplexer — i.e. it's likely a TUI app.
    private func hasNonShellDescendant(parentPid: pid_t, maxDepth: Int) -> Bool {
        var queue: [(pid: pid_t, depth: Int)] = [(parentPid, 0)]
        var foundNonShellLeaf = false
        ...
        while !queue.isEmpty {
            ...
            let count = byteCount > 0 ? Int(byteCount) / MemoryLayout<pid_t>.stride : 0

            if count == 0 && depth > 0 {
                // Leaf process — check if it's a shell
                var nameBuffer = [CChar](repeating: 0, count: Int(MAXCOMLEN) + 1)
                proc_name(pid, &nameBuffer, UInt32(nameBuffer.count))
                let name = String(cString: nameBuffer)
                if !name.isEmpty && !knownShells.contains(name) {
                    foundNonShellLeaf = true
                    break
                }
            }
    ...
    private func detectInjectionMethod() -> (method: InjectionMethod, textMethod: TextSendingMethod) {
        ...
        if fastTerminals.contains(bundleID) || slowTerminals.contains(bundleID) {
            if canScanForTUI && isTerminalRunningTUI() {
    ...
    func injectSync(backspaceCount: Int, text: String, proxy: CGEventTapProxy?) {
        ...
        if method == .extraSlow {
            ...
            usleep(15000) // 15ms settle for TUI render
            return
        }
        ...
        // Settle time
        let settleTime: UInt32 = (method == .slow) ? 15000 : 3000
        usleep(settleTime)
    }
    ...
    private func sendAtomicReplacement(backspaceCount: Int, text: String) {
        ...
        while offset < utf16.count {
            ...
            if offset < utf16.count {
                usleep(4000) // 4ms between chunks
            }
        }
    }
